...
def imprimir_laberinto(camino_actual):
    print("\nEstado actual:")
    for f in range(9):
        fila_str = ""
        for c in range(9):
            if (f, c) in camino_actual:
                # El asterisco centrado en 3 espacios
                fila_str += f"{'*':>3}"
            else:
                # Cada número alineado a la derecha en 3 espacios
                fila_str += f"{laberinto[f][c]:>3}"
        print(fila_str)
